...
to setup
  ...
  set species n-values nr-of-species [ ? ] ; 0, 1, 2, ..., n-1
  create-pens nr-of-species [ 
    set shape "circle"
    set size 0.5
    set color item who map [ ? + 0 ] colours
    set pen-size 1
  ]
  foreach but-first species [
    create-temporary-plot-pen (word "p" (? + 1))
    set-plot-pen-color item ? colours
  ]
 ; print proportions
  steps
end
...
to-report some-reward-matrix
  ...
  if fitness-matrix = "custom" [
    ...
    if not is-list? matrix or not empty? filter [ not is-list? ? ] matrix [
...
to go
  foreach species [
    set-current-plot-pen (word "p" (? + 1))
    plot item ? proportions
  ]
  ask pens [
    let me item who proportions
    let some-others sum map [ item ((who + ? + 1) mod nr-of-species) proportions ] n-values S [ ? ]
  ...
  set unnormalised-proportions map [ (item ? proportions) * score ? ] species
  ;print "---------------------"
  ;print map [ score ? ] species
...
; 
to-report score [ i ]
  let pure-score sum map [ (item ? proportions) * (item ? (item i reward-matrix)) ] species
