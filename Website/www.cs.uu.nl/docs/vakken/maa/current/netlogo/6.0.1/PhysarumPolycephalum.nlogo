; Use vid extension to replace deprecated movie functions
extensions [vid]
...
to toggle-record-movie
  if-else not recording-movie? [
    ...
    set close-movie-at-tick 60 * duration-in-minutes * 25
    vid:start-recorder

    ; function deprecated, all movies (vids now) run at 25 frames per second
    ; movie-set-frame-rate frames-per-second
  ]
  [
    if-else user-yes-or-no? "Do you want to save this movie and it's parameters?"
    [
      vid:save-recording (word export-prefix ".mov")
      print-screen
      set recording-movie? false
      stop
    ]
...
sMin
sMin
0 0.1 0.001 1.0E-4 1.0E-5 1.0E-6 1.0E-7 1.0E-8 1.0E-9
0
...
Ant variables:
  ...
  * **SA**
      : Sensory angle of the ant

  * **RA**
      : Rotation angle of the ant

  * **SO**
      : Sensory offset distance

  * **sMin**
    : sensitivity threshold of sensing pheromone

  * **SW**
    (not shown in interface) : Sensory width
