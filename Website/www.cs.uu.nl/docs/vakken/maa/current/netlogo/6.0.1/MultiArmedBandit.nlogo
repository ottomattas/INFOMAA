...
to setup
  ...
  set all-actions n-values (max-pxcor + 1) [ ?1 -> ?1 ]
  ;
  ...
  set devn-of array:from-list map [ ?1 -> random-float (?1 / sqrt(3)) ] array:to-list mean-of
  ;
...
to c [ x ]
  ...
  set action-random  z
  color-patches
  set realisation map [ ?1 -> give-immediate-reward ?1 ] all-actions
...
to-report y ; computer player
  ...
  report epsilon-greedy map [ ?1 -> array:item geo-com ?1 ] all-actions
end
...
to print-mean-payoffs
  clear-output
  foreach all-actions [ ?1 ->
    output-print (word "c " (?1 + 1) ": mu = " precision array:item mean-of ?1 1)
  ]
end
@#$#@#$#@
GRAPHICS-WINDOW
