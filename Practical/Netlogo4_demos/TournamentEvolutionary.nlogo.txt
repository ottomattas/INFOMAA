...
to setup
  ...
  set strategy-indexes n-values nr-of-strategies [ ? ]
  output-print (word
  ...
  foreach strategy-indexes [
    ask patch (? + 1) (0 - nr-of-strategies - 1) [
      write-proportion ?
      set plabel (precision item ? proportion 1) set plabel-color cyan
    ]
    set x expected-score-of ?
  ]
  define-stop-light
end
...
to-report create-partition [ n ]
  ...
  report map [ ? / sum answer ] answer
end

to go
  foreach strategy-indexes [
    write-proportion ?
    set-current-plot-pen item ? strategies
    plot item ? proportion
  ]
  set proportion-times-score map [ (item ? proportion) * (expected-score-of ?) ] strategy-indexes
  let total-score sum map [ ? ] proportion-times-score
  let old-proportion proportion
  set proportion map [ ? / total-score ] proportion-times-score
  tick
  if stop-if-no-change [
    let delta sum (map [ (?1 - ?2) ^ 2 ] proportion old-proportion)
    if delta < 1.0E-20 [
      output-print (word "Stopped at round " ticks " because of convergence.")
      let ranking map [ item ? strategies ] sort-by [ item ?1 proportion > item ?2 proportion] strategy-indexes
      output-print (word "Winner: " reduce [ (word ?1 ",\nthen " ?2) ] ranking ".")
      set-stop-light red
      stop
    ]
  ]
  if ticks mod nr-of-gens-before-stop = 0 [
    ...
    let ranking map [ item ? strategies ] sort-by [ item ?1 proportion > item ?2 proportion] strategy-indexes
    output-print (word "Ranking so far: " reduce [ (word ?1 ",\nthen " ?2) ] ranking ".")
...
to-report expected-score-of [ strategy-index ]
  let expected-score sum map [
    (item ? proportion) * (item (nr-of-strategies * strategy-index + ?) relative-score)
  ] strategy-indexes
