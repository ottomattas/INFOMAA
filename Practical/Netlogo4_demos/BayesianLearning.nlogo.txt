...
globals [ classes actions history-pro history-con dump-interface ]

turtles-own [ history-probability class prob ]
...
to setup
  ...
  set classes [
    ...
    ["all-D"                   red       true ]
    ["C-10"                    pink       true ]
    ["C-90"                    43        false] ; cooperate with probability 0.5
    ["unforgiving"             102       true ] ; cooperate util opponents defects; then defect forever
    ["tit-for-tat"             violet    true ]
  ...
  set history-pro []
  set history-con []
  foreach classes [create-class ?1]
  ask turtles [ visualize ]
  set dump-interface true
  if dump-interface [
...
to create-class [ cls ]
  crt 1 [ 
    ...
    set-current-plot "P{ history | strategy }"
    if-else who = 0 [
    ...
    set-current-plot "P{ strategy | history } = P { h | s } * P{ s } / P{ h }"
    if-else who = 0 [
      set-current-plot-pen class
    ] [
      create-temporary-plot-pen class
...
to visualize
  ask patches with [
    ...
    set pcolor ifelse-value (pycor < xxx) [ [ color ] of myself ] [ black ]
  ]
  ask patch who who [ set plabel precision [ history-probability ] of myself 10 ]
  set-current-plot "P{ history | strategy }"
  set-current-plot-pen class
  ...
  set-current-plot "P{ strategy | history } = P { h | s } * P{ s } / P{ h }"
  set-current-plot-pen class
  plot history-probability / sum [ history-probability ] of turtles
end

; -------------------------------------------------------------

to-report all-C
  report ifelse-value (item 0 history-con = 0) [ 1   ] [ 0   ] 
end
...
to-report unforgiving
  ...
  if member? 1 but-first history-pro and not (item 0 history-con = 1) [ report 0 ]
  report 1
end
...
to-report majority
  let count-C length filter [ ? = 0 ] but-first history-pro
  let count-D length filter [ ? = 1 ] but-first history-pro
...
909
457
P{ strategy | history } = P { h | s } * P{ s } / P{ h }
NIL
NIL
0.0
10.0
