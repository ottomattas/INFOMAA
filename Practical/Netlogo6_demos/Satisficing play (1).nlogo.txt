...
to setup
  ...
  ifelse uniform-initial-aspiration [ create-couples-randomly ] [ create-couples-uniformly ]
  ask players [
...
to randomise
  ...
  foreach [ 0 1 2 ] [ ?1 ->
    ...
    foreach [ 0 1 2 ] [ ??1 ->
      ...
      create-outcomes 1 [
        set visits 0
        if-else display-profile-visits [
          set shape "blank" set size 0.5 set color black
        ] [
...
to-report rd
  report ifelse-value int-payoffs [ random 10 ] [ precision (random-float 10) 2 ]
end
...
to-report random-but [ k n ]
  ...
  let new-k ifelse-value force-switch [ one-of remove-item k n-values n [ ?1 -> ?1 ] ] [ random n ]
...
to-report perturb-matrix [ matrix ]
  report map [ ?1 -> map [ ??1 -> precision (random-uniform ??1 perturb) 2 ] ?1 ] matrix
end
...
false
0
Polygon -7500403 true true 0 75 255 75 255 225 0 225
