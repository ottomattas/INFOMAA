globals [ bugs-are-tracked centroids-are-tracked centroids-are-shown links-are-shown ]
...
to setup
  ...
  ask bugs [
    set color item (who mod 5) [yellow orange lime cyan pink]
    hatch-centroids 1 [
      set hidden? true
      set shape "target" ; "x"
  ...
  set bugs-are-tracked      not track-bugs
...
to read-sliders
  if bugs-are-tracked != track-bugs [
    ask bugs [ ifelse track-bugs [ pd ] [ pu ] ]
    set bugs-are-tracked track-bugs
  ]
...
to-report done
  let mean-average-distance-to-others mean [ average-distance-to-others ] of bugs
  let max-average-distance-to-others max [ average-distance-to-others ] of bugs
  let min-average-distance-to-others min [ average-distance-to-others ] of bugs
...
to-report other-bugs-i-perceive
  let other-bugs other bugs
  if adapt-to-all-others [ report other-bugs ]
  let far-bugs nobody
  if-else attraction > 0 [
    set far-bugs other-bugs with [ distance myself >= neighbourhood ]
    if not any? far-bugs [ report other-bugs ]
  ] [
    set far-bugs other-bugs
  ]
  if count far-bugs <= number-closest-to-adapt-to [ report far-bugs ]
  if-else remote [
    report max-n-of number-closest-to-adapt-to far-bugs [ distance myself ]
  ] [
    report min-n-of number-closest-to-adapt-to far-bugs [ distance myself ]
  ]
end

to-report target-heading
  if who = 0 and teach-by-mouse [ report towardsxy mouse-xcor mouse-ycor ]
  let ot other-bugs-i-perceive
...
to-report average-distance-to-others
  report mean [ distance myself ] of other bugs
end
